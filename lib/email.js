...
async function sendViaSmtp({ to, subject, html, text }) {
  ...
  await transport.sendMail({ from: EMAIL_FROM, to, subject, html, text, replyTo: EMAIL_FROM });
...
async function sendViaResend({ to, subject, html, text }) {
  ...
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({ from: EMAIL_FROM, to, subject, html, text, reply_to: EMAIL_FROM })
  });
  if (!res.ok) {
...
export function sendOtpEmail(to, otp, { purpose = 'verify' } = {}) {
  ...
  const text = `Carto\n\n${title}\n\n${intro}\n\nYour code: ${otp}\n\nNever share this code with anyone. Carto staff will never ask for it.\n\nIf you didn't request this, you can safely ignore this email.`;
  return sendEmail({ to, subject: isReset ? 'Your Carto password reset code' : 'Your Carto verification code', html, text });
}
